                ...
                <div v-else >
                    <div  v-for="item in this.comment.slice((pageNum-1)*pageSize, pageNum*pageSize)" >
                        <b style="color: #52c41a;font-size: 17px ">{{item.from.username}}
                            ...
                            <Modal v-model="modal3" title="添加评论" ok-text="确定" cancel-text="取消" @on-ok="reply_comment(item)"
                                   @on-cancel="cancel" >
                                <textarea v-model="content" placeholder="写下你的想法" style="margin-left: 4%;width:90%;height: 200px"/>
                            </Modal>
                        </b>
...
<script>
    ...
    export default {
        ...
        methods:{
            ...
            //输入：from_email、comment_id、to_email（被回复者ID)、content、from_name
            reply_comment(item){
                if(this.content=='')
                    return;
                else {
                    let params = {'from_email':this.GLOBAL.email,'comment_id':item.comment_id,'to_email':item.from.userid,
                        'content':this.content,'from_name':this.GLOBAL.userName};
